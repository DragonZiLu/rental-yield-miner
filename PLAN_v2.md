...
| `price_change_est_pct` | float | -50 ~ +50 | 估算12月变动幅度(%) |
| `price_vs_peak_pct` ⭐ | float | -100 ~ 0 | 相对历史最高点的跌幅(%)，如 -25 = 已从最高跌25% |
| `peak_year` ⭐ | int | 年份 | 历史最高点对应的年份（如 2021） |
...
### 4.4 价格回撤深度分析（基于 price_vs_peak）

```python
def price_depth_analysis(sentiment, population):
    """
    价格距离历史高点的回撤幅度辅助判断
    """
    drawdown = abs(sentiment.price_vs_peak_pct)  # 正值，如 18%
    peak_year = sentiment.peak_year
    years_since_peak = 2026 - peak_year
    
    if drawdown >= 30 and population.population_trend == "strong_inflow":
        return "🐻 超跌+强需求：可能已过度反应，安全边际极高"
    elif drawdown >= 20 and population.population_trend != "outflow":
        return "📉 深度回调：价格已回吐较多泡沫，关注企稳信号"
    elif drawdown >= 10:
        return "📊 温和回调：正常周期调整"
    else:
        return "📈 接近高点：租售比可能被价格高企压缩"
```

**距离历史高点越远 + 人口越强 = 安全边际越大**
...
╠═══════╤══════════╤══════╤══════════╤══════════════╤══════╤══════╤════════╤════════╗
║ 城市  │ 人口趋势 │年流入 │ 12月趋势 │ 距历史高点    │情绪  │ 悲观 │ 达标数  │ 判断   ║
╠═══════╪══════════╪══════╪══════════╪══════════════╪══════╪══════╪════════╪════════╣
║ 成都  │ 强流入   │+12万 │ ↓-8%    │ ↓-18%(21年)  │磨底  │  70  │  8个    │ 🟢关注 ║
║ 杭州  │ 强流入   │+15万 │ ↓-10%   │ ↓-22%(21年)  │下跌  │  68  │  3个    │ 🟢关注 ║
║ 深圳  │ 流入     │ +8万 │ ↓-12%   │ ↓-35%(20年)  │磨底  │  65  │  2个    │ 🟢关注 ║
║ 武汉  │ 流入     │ +5万 │ ↓-15%   │ ↓-28%(19年)  │恐慌  │  85  │  4个    │ 🟡等待 ║
║ 天津  │ 弱流入   │ +1万 │ ↓-8%    │ ↓-25%(17年)  │下跌  │  72  │  3个    │ 🟡谨慎 ║
║ 西安  │ 稳定     │ +2万 │ ↓-6%    │ ↓-12%(22年)  │下跌  │  60  │  3个    │ 🟡中性 ║
║ ...   │ ...      │ ...  │ ...      │ ...           │ ...  │ ...  │ ...     │ ...    ║
╚═══════╧══════════╧══════╧══════════╧══════════════╧══════╧══════╧════════╧════════╝
```

> 关键解读：深圳距 2020 年高点跌 35%，如果人口还在流入，属于"超跌+基本面好"的黄金坑特征。
